...
from PyQt6.QtWidgets import QListWidgetItem, QTableWidgetItem, QMessageBox, QMainWindow, QTableWidget

from libs.JsonFileFactory import JsonFileFactory
from libs.libsPro.DataConnector import DataConnector
from libs.libsPro.ExportTool import ExportTool
from models.Product import Product
from uis.uiPro.ProductMainWindow import Ui_MainWindow
...
class ProductMainWindowExt(QMainWindow,Ui_MainWindow):
    def __init__(self,menu_window):
        ...
        self.setupUi(self)
        self.tableWidgetProduct.setSelectionMode(QTableWidget.SelectionMode.MultiSelection)
        self.tableWidgetProduct.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows)
    ...
    def setupSignalAndSlot(self):
        ...
        self.pushButtonDelete.clicked.connect(self.delete_products)
        ...
        self.listWidgetCategory.itemClicked.connect(self.filter_products_by_category)

    def filter_products_by_category(self, item):
        selected_cateid = item.text()
        self.products = [p for p in self.all_products if p.cateid == selected_cateid]
        self.show_products_gui()
    ...
    def save_product(self):
        ...
        if index == -1:
            ...
            QMessageBox.information(self.MainWindow, "Thành công", "Sản phẩm mới đã được thêm!")
        else:
            self.products[index] = product
            QMessageBox.information(self.MainWindow, "Thành công", "Sản phẩm đã được cập nhật!")
        jff = JsonFileFactory()
    ...
    def delete_products(self):
        """
        Xóa một hoặc nhiều sản phẩm được chọn trong bảng sản phẩm (QTableWidget).
        """
        selected_rows = set()  # Dùng set để tránh trùng dòng
        selected_items = self.tableWidgetProduct.selectedItems()

        if not selected_items:
            QMessageBox.warning(self, "Thông báo", "Vui lòng chọn ít nhất một sản phẩm để xóa!")
            return

        # Lấy danh sách dòng đã chọn
        for item in selected_items:
            selected_rows.add(item.row())

        # Lấy danh sách mã sản phẩm từ các dòng đã chọn
        proids = []
        for row in selected_rows:
            product_id_item = self.tableWidgetProduct.item(row, 0)  # Cột 0 là Product ID
            if product_id_item:
                proids.append(product_id_item.text())

        if not proids:
            QMessageBox.warning(self, "Lỗi", "Không lấy được ID sản phẩm để xóa!")
            return
        ...
        confirm = QMessageBox.question(
            self,
            "Xác nhận xóa",
            f"Bạn có chắc chắn muốn xóa {len(proids)} sản phẩm không?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )

        if confirm == QMessageBox.StandardButton.Yes:
            success = self.dc.delete_products(proids)  # Gọi hàm xóa trong DataConnector
            if success:
                QMessageBox.information(self, "Thông báo", "Xóa sản phẩm thành công!")
                self.products=self.dc.get_all_products()
                self.show_products_gui()  # Cập nhật lại danh sách sản phẩm
            else:
                QMessageBox.warning(self, "Lỗi", "Không tìm thấy sản phẩm để xóa!")
    ...
    def import_from_excel(self):
        ...
        self.listWidgetCategory.clear()
        for cate in self.categories:
            self.listWidgetCategory.addItem(str(cate.cateid))  # Hiển thị Cate ID
